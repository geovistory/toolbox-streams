...
public class CommunityTopStatementAdder {
    public static List<CommunityStatementValue> addStatement(List<CommunityStatementValue> existingList, CommunityStatementValue newItem, boolean isOutgoing) {
        int targetPosition = -1;
        int replacedItemPosition = -1;

        for (int i = 0; i < existingList.size(); i++) {
            var oldItem = existingList.get(i);
            var oldOrdNum = isOutgoing ? oldItem.getAvgOrdNumOfRange() : oldItem.getAvgOrdNumOfDomain();
            var newOrdNum = isOutgoing ? newItem.getAvgOrdNumOfRange() : newItem.getAvgOrdNumOfDomain();
        ...
        // if item is in no project...
        if (newItem.getProjectCount() < 1) {
            // ...remove it
