...
class ProjectEntityLabelRecursionTest {
    ...
    private TestInputTopic<dev.information.statement.Key, StatementEnrichedValue> statementEnrichedTopic;
    ...
    void setup() {
        ...
        statementEnrichedTopic = testDriver.createInputTopic(
...
/*
        inputProjectEntityLabelTopic = testDriver.createInputTopic(
...
*/
    ...
    void testProjectEntityLabel() {
        ...
        var entityFoo = "person";
        var entityOneId = "name1";
        var entityTwoId = "name2";

        // add statement one
        var kS = dev.information.statement.Key.newBuilder().setPkEntity(statementOneId).build();
        var vS = StatementEnrichedValue.newBuilder().setSubjectId(entityFoo).setPropertyId(propertyId).setObjectId(entityOneId).build();
        statementEnrichedTopic.pipeInput(kS, vS);
        ...
        vS = StatementEnrichedValue.newBuilder().setSubjectId(entityFoo).setPropertyId(propertyId).setObjectId(entityTwoId)
                .build();
        statementEnrichedTopic.pipeInput(kS, vS);
        ...
        vS = StatementEnrichedValue.newBuilder().setSubjectId(entityOneId).setPropertyId(propertyHasStringId)
                .setObjectLabel("Name 1")
                .setObjectLiteral(LiteralValue.newBuilder().setLabel("Name 1").setId("").setClassId(0).build()).build();
        statementEnrichedTopic.pipeInput(kS, vS);
        ...
        vS = StatementEnrichedValue.newBuilder().setSubjectId(entityTwoId).setPropertyId(propertyHasStringId)
                .setObjectLabel("Name 2")
                .setObjectLiteral(LiteralValue.newBuilder().setLabel("Name 2").setId("").setClassId(0).build()).build();
        statementEnrichedTopic.pipeInput(kS, vS);
        ...
        var kE = ProjectEntityKey.newBuilder().setEntityId(entityFoo).setProjectId(projectId).build();
        var vE = ProjectEntityValue.newBuilder().setEntityId(entityFoo).setProjectId(projectId).setClassId(classPersonId).build();
        projectEntityTopic.pipeInput(kE, vE);

        // add project entity one
        kE = ProjectEntityKey.newBuilder().setEntityId(entityOneId).setProjectId(projectId).build();
        vE = ProjectEntityValue.newBuilder().setEntityId(entityOneId).setProjectId(projectId).setClassId(classNameId).build();
        projectEntityTopic.pipeInput(kE, vE);

        // add project entity two
        kE = ProjectEntityKey.newBuilder().setEntityId(entityTwoId).setProjectId(projectId).build();
        vE = ProjectEntityValue.newBuilder().setEntityId(entityTwoId).setProjectId(projectId).setClassId(classNameId).build();
        projectEntityTopic.pipeInput(kE, vE);
        ...
        var resultingKey = ProjectEntityKey.newBuilder().setEntityId(entityTwoId).setProjectId(projectId).build();
        var record = outRecords.get(resultingKey);
        assertThat(record.getLabel()).isEqualTo("Name 2");

        resultingKey = ProjectEntityKey.newBuilder().setEntityId(entityFoo).setProjectId(projectId).build();
